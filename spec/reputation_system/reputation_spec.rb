...
describe ActiveRecord::Base do
  ...
  context "Contribution" do
    describe "#contribution_value_for" do
      it "should return 1 as if there is no data" do
        @question.contribution_value_for(:total_votes).should == 1
      end
      ...
      it "should return appropriate value in case of valid input" do
        ...
        question2.add_evaluation(:total_votes, 2, @user)
        question3.add_evaluation(:total_votes, 3, @user)
        @question.contribution_value_for(:total_votes).should == 1.to_f/6
        question2.contribution_value_for(:total_votes).should == 0.5
        question3.contribution_value_for(:total_votes).should == 1
      end

      it "should raise exception if invalid reputation name is given" do
        lambda {@question.contribution_value_for(:invalid)}.should raise_error(ArgumentError)
      end

      it "should raise exception if scope is given for reputation with no scopes" do
        lambda {@question.contribution_value_for(:difficulty, :s1)}.should raise_error(ArgumentError)
      end

      it "should raise exception if scope is not given for reputation with scopes" do
        lambda {@phrase.contribution_value_for(:difficulty_with_scope)}.should raise_error(ArgumentError)
      end
    end
  end
end
